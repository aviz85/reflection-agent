...
for i in 1 2 3; do
    ...
    if [ -f "$DIR/findings/$prev_date.md" ]; then
        PREV_FINDINGS="$PREV_FINDINGS$(cat "$DIR/findings/$prev_date.md")\n\n---\n\n"
    fi
done
...
$(cat "$TMP/today-prompts.txt" 2>/dev/null || echo "(no activity)")
...
$(head -30 "$TMP/project-activity.tsv" 2>/dev/null)
...
$(cat "$TMP/skill-usage.txt" 2>/dev/null)
...
$(echo -e "$PREV_FINDINGS" 2>/dev/null || echo "(first run)")
...
claude -p "$PATTERN_PROMPT" --model claude-sonnet-4-6 --max-turns 1 > "$TMP/patterns.md" 2>/dev/null || echo "Phase 2 failed" > "$TMP/patterns.md"
...
QUESTION_PROMPT="You are a CURIOUS research agent. Not analytical — genuinely curious.
...
## Open Questions (DO NOT repeat these):
$(cat "$DIR/questions/open.md")

## Context:
$(head -100 "$TMP/context-digest.txt" 2>/dev/null)
...
claude -p "$QUESTION_PROMPT" --model claude-sonnet-4-6 --max-turns 1 > "$DIR/questions/$DATE.md" 2>/dev/null || echo "# Questions — $DATE\n\n(generation failed)" > "$DIR/questions/$DATE.md"
...
if grep -qi "repetition\|repeated\|חוזר\|reflected light" "$TMP/patterns.md" 2>/dev/null; then
    ...
    SKILL_PROMPT="A repeated pattern was detected in Aviz's workflow:

$(grep -A5 -i "repetition\|repeated\|reflected" "$TMP/patterns.md" | head -20)
...
Only output the SKILL.md content. Be practical and concise."
    ...
    claude -p "$SKILL_PROMPT" --model claude-sonnet-4-6 --max-turns 1 > "$DIR/skills-proposed/$DATE-proposal.md" 2>/dev/null || true
...
SYNTH_PROMPT="You are the Reflection Agent. Synthesize today's analysis into 3 outputs.

## Patterns Found:
$(cat "$TMP/patterns.md")

## Questions Generated:
$(cat "$DIR/questions/$DATE.md" 2>/dev/null)

## Skill Proposals:
$(cat "$DIR/skills-proposed/$DATE-proposal.md" 2>/dev/null || echo "None today")

## Current CLAUDE.md:
$(cat "$DIR/CLAUDE.md")

Generate exactly 3 sections separated by '---SECTION---':
...
SYNTH_RESULT=$(claude -p "$SYNTH_PROMPT" --model claude-sonnet-4-6 --max-turns 1 2>/dev/null || echo "Synthesis failed")
...
$line" "$DIR/CLAUDE.md" 2>/dev/null || true
    ...
    if [ "$total_lines" -gt 200 ]; then
        echo "[Warning] CLAUDE.md exceeded 200 lines ($total_lines). Truncating Learned Patterns."
...
const opts = {hostname:'localhost', port:3033, path:'/api/sendText', method:'POST', headers:{'Content-Type':'application/json'}};
